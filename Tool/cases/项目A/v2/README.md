...
## 7. ⚠ 复现踩坑记录：字符串全局不可用 BSTR 直写（v2.1 修正）

首次在真实安装环境复现时，进程在 18 秒内以 **`0xC0000374`（STATUS_HEAP_CORRUPTION）** 退出。

**根因**：把宿主自带的 `OLEAUT32!SysAllocString` 返回的 **BSTR** 直接写入宿主的字符串全局变量，
而宿主按**自有内存管理器**（Delphi UnicodeString 语义）复制/释放该指针 → 堆管理器记账错乱。
第三方破解件（v1 分析对象）同样使用该手法，因此同样存在该崩溃风险 —— 这解释了破解件"时好时坏"的现象。

**修正（v2.1）**：

1. **默认只写授权状态**（`license_type = 5`、`ver_flag = 1`），**不触碰字符串全局** → 实测稳定 60 秒以上无异常；
2. 字符串写入改为**显式开关**：在 DLL 同目录创建空文件 `version_poc_strings.enable` 才会启用；
3. 许可证名/码改为**由宿主自身从配置文件读取**（配置文件伪造法，方式 B），避免跨内存管理器写指针；
4. 启动前 8 秒改为 **50 ms 高频抢占**（原 1.5 s 延迟 + 500 ms），确保赶在宿主计算授权标签之前落地。

**修正后实测（真实安装 28.40.0100）**：

| 观测项 | 结果 |
| :--- | :--- |
| `license_type` | **5（Lifetime License）** ✅ |
| `ver_flag` | 1 ✅ |
| 许可证码全局 | `xy05-Lifetime-License-Pro-poc-user`（由宿主自身载入）✅ |
| 进程稳定性 | 60 s+ 无崩溃 ✅ |

> 教训沉淀：**跨内存管理器的指针写入是注入类 PoC 的隐形杀手**；优先"让宿主自己写"，而非"替宿主写"。
